...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        "*** YOUR CODE HERE ***"
        if legalActions:
            if util.flipCoin(self.epsilon):  # exploration: choose randomly the action
                action = random.choice(legalActions )
            else:
                action = self.computeActionFromQValues(state) # exploitation: choose the best action
        return action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        w = self.getWeights()
        q_value =  self.featExtractor.getFeatures(state, action) * w
        return q_value
